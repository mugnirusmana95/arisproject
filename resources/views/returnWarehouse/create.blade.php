    ...
    <form class="form-horizontal" method="post" action="{{route('gi.return.warehouse.store')}}">
      ...
      <div class="box-body">
        ...
        <div class="form-group {{$errors->has('gow') ? 'has-error' : ''}}">
          <label for="gow" class="control-label col-md-2">ID Barang Keluar <span class="req">*</span></label>
          <div class="col-md-10">
            <select class="form-control" id="gow" name="gow">
              <option value=""></option>
            </select>
            <p class="help-block">
              @if ($errors->has('gow'))
...
$(document).ready(function(){
  $("#gow").select2({
    ...
    width: "100%",
    ajax: {
      url: '{{route('go.warehouse.getIdReturn')}}',
      dataType: 'json',
